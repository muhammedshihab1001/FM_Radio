...
const STATUS_LABEL: Record<string, string> = {
  ...
  stalled:       'Weak Connection',
  'mixed-content': 'Connection Blocked (HTTP)',
};
...
export const MiniPlayer: React.FC<MiniPlayerProps> = React.memo(({ 
  ...
  return (
    <div className="relative group/player animate-slide-up pointer-events-auto w-full">
      ...
      <div className="relative h-16 md:h-20 px-4 md:px-6 rounded-[1.5rem] md:rounded-[2rem] bg-black/70 border border-white/10 backdrop-blur-3xl flex items-center justify-between shadow-2xl overflow-hidden group/dock mb-[env(safe-area-inset-bottom)]">
        <div className="absolute inset-x-0 bottom-0 h-1 bg-gradient-to-r from-transparent via-cyan-500/30 to-transparent" />

        <div className="flex items-center gap-3 md:gap-4 flex-1 min-w-0">
          <div 
            className="w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center shrink-0 overflow-hidden relative group/logo"
            title={isPlaying ? "Broadcast Active" : "No Broadcast"}
          >
              {isPlaying && !isErr ? (
          ...
          <div className="min-w-0">
            ...
            <div 
              className="flex items-center gap-1.5 mt-0.5 md:mt-1 cursor-help"
              title={label}
            >
              <span className={`w-1 h-1 md:w-1.5 md:h-1.5 rounded-full ${isErr || status === 'mixed-content' ? 'bg-red-500' : isLive ? 'bg-cyan-400 animate-pulse shadow-[0_0_8px_rgba(34,211,238,0.8)]' : 'bg-white/20'}`} />
              <p className="text-[8px] md:text-[10px] font-mono font-bold text-white/40 uppercase tracking-[0.1em] md:tracking-[0.2em] truncate">
                {status === 'mixed-content' ? 'Connection Blocked (HTTP)' : `${label} • ${station.country || 'Global'}`}
              </p>
            </div>
          </div>
        </div>

        <div className="hidden xl:flex flex-1 items-center gap-1.5 mx-12 h-10 max-w-sm">
          {[...Array(24)].map((_, i) => (
            <div
              key={i}
        ...
        <div className="flex items-center gap-2 md:gap-4 shrink-0">
          <div className="flex items-center">
            ...
            <div 
              className="hidden md:flex w-8 h-8 items-center justify-center text-white/20"
              title="Broadcast Volume"
            >
          ...
          <button
            onClick={onToggle}
            className="w-11 h-11 md:w-14 md:h-14 rounded-xl md:rounded-2xl bg-gradient-to-br from-cyan-500 to-blue-500 p-[1px] group/play active:scale-95 transition-all shadow-lg shadow-cyan-500/10"
